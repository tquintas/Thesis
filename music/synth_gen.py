...
def CreateRandomPercKeyboard(name="RandomPerc"):
    t0 = _t.time()
...
def CreateRandomWavePercKeyboard(wave_class=_r.choice(list(waves.values())), name="RandomWave"):
    t0 = _t.time()
    folder = f"{bitSamples}/{name}"
    try:
        _p(folder).mkdir(parents=True)
